...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    if (self) { self.backgroundColor = [UIColor clearColor]; self.layer.cornerRadius = 6.0; self.layer.masksToBounds = YES; }
    return self;
}
- (void)drawRect:(CGRect)rect {
    CGContextRef ctx = UIGraphicsGetCurrentContext();
    if (!ctx) return;
    int steps = (int)CGRectGetWidth(rect); if (steps < 1) steps = 1;
    CGFloat stepW = CGRectGetWidth(rect) / steps;
    for (int i = 0; i < steps; i++) {
        CGContextSetFillColorWithColor(ctx, [[UIColor colorWithHue:(CGFloat)i/steps saturation:1.0 brightness:0.85 alpha:1.0] CGColor]);
...
- (void)setHue:(CGFloat)hue {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"BINFlashHueChangedNotification" object:@((double)hue)];
}
- (CGFloat)hue {
